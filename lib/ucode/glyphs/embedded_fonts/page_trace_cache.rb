...
module Ucode
  module Glyphs
    module EmbeddedFonts
      ...
      #
      # Replaces the O(F × P) explosion in the pre-TODO-10
      # TraceStrategy, which spawned mutool trace once per page
      # per CID font. For Code Charts PDFs with F trace-needing
      # fonts and P pages, that was F×P subprocess invocations;
      # this cache makes it exactly P invocations.
      #
      # Why per-page grouping: TraceCorrelator's algorithm clusters
      # label glyphs by Y bucket, and Y positions are page-local
      # (page 2's coordinate system is independent of page 1's).
      # Grouping by page preserves the per-page correlation
      # semantics of the pre-cache path.
      #
      # Lazy: glyphs are only fetched when a strategy first asks
      # for them. PDFs where every font has /ToUnicode never
      # trigger the trace at all (zero invocations).
      class PageTraceCache
        ...
        def each_page_for(base_font)
          ...
          glyphs_by_page.each_with_index do |glyphs, idx|
            next if idx.zero? # index 0 unused (1-based pages)
