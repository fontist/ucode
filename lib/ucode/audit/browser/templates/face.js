...
(function () {
  ...
  function renderBlocks() {
    ...
    blocks.forEach(function (block, i) {
      ...
      html += '<td class="num">' + (block.range || '') + '</td>';
      html += '<td class="num">' + block.covered_count + '</td>';
      html += '<td class="num">' + block.total_assigned + '</td>';
      html += '<td class="num">' + (block.coverage_percent || 0).toFixed(1) + '%</td>';
      html += '<td><span class="status status-' + block.status + '">' + block.status + '</span></td>';
      html += '</tr>';
  ...
  function renderMissingChips(block) {
    ...
    var html = '<p class="hint">Missing (' + missing.length + '):</p>';
    if (!missing.length) return html;
    html += '<div class="cp-chips">';
    missing.slice(0, 200).forEach(function (cp) {
      html += '<span class="cp-chip" data-cp="' + cp + '">U+' +
              cp.toString(16).toUpperCase().padStart(4, "0") + '</span>';
    });
    if (missing.length > 200) {
      html += '<span class="cp-chip">… +' + (missing.length - 200) + ' more</span>';
    }
    html += '</div>';
    return html;
  }
    ...
    fetchCodepointChunk(blockName).then(function (chunk) {
      ...
      if (!detail) {
        panel.innerHTML = '<div class="detail-panel">No detail for U+' +
                          cp.toString(16).toUpperCase() + '</div>';
        return;
      }
