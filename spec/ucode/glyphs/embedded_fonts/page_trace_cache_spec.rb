...
class StubTraceMutool
  ...
  # @param by_page [Hash{Integer=>String}] canned XML keyed by page number
  ...
  def call(pdf, *pages)
    @calls << [pdf, pages]
    pages.map { |p| @by_page.fetch(p) { "" } }.join
  end
end
...
RSpec.describe Ucode::Glyphs::EmbeddedFonts::PageTraceCache do
  ...
  describe "#glyphs_by_page" do
    ...
    it "returns [[]] when page_count is zero (no tracing)" do
  ...
  describe "#each_page_for" do
    it "yields (page, glyphs) for every page that references the font" do
      # Each yielded +glyphs+ is the FULL page's glyphs (all fonts),
      # not just the requested font's — TraceCorrelator filters
      # internally (specimens vs labels).
      mutool = StubTraceMutool.new(
        by_page: {
          1 => <<~XML,
            <document>
      ...
      yielded_b = []
      cache.each_page_for("B") { |p, g| yielded_b << [p, g.map(&:gid)] }
      expect(yielded_b).to eq([[2, [2]], [3, [3, 4]]])
    end

    it "returns true when at least one page references the font" do
      mutool = StubTraceMutool.new(
        by_page: { 1 => '<document><span font="A"><g glyph="1"/></span></document>' },
      )
      cache = described_class.new(pdf: pdf, page_count: 1, mutool: mutool)

      result = cache.each_page_for("A") { |_| }
    ...
    it "returns an Enumerator when no block given" do
      ...
      expect(enum.to_a.first[0]).to eq(1) # [page, glyphs]
    end
  end
